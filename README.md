...
4. **对于纯命令行/无桌面的服务器（如云主机/Debian Server）**：在终端提示时输入 `xvfb`。脚本将全自动为您在后台完成以下操作：
   - 创建 `:99` 虚拟显示器 (Xvfb)
   - 启动轻量级窗口管理器 (Fluxbox)，以防 RustDesk 没有标题栏无法移动
   - 开启无密码的 VNC 远程桌面服务 (x11vnc)
   - *随后您只需用本地电脑上的 VNC Viewer 连入服务器 IP 的 `5900` 端口，在虚拟桌面中打开 RustDesk 连上被控机，然后回车让脚本接管即可。*
